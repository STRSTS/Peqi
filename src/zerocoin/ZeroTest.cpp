...
#define TEPQI_COINS_TO_ACCUMULATE   10
...
// Global coin array
PrivateCoin    *gCoins[TEPQI_COINS_TO_ACCUMULATE];
...
Test_Accumulator()
{
	...
	try {
		...
		for (uint32_t i = 0; i < TEPQI_COINS_TO_ACCUMULATE; i++) {
			accOne += gCoins[i]->getPublicCoin();
			accTwo += gCoins[TEPQI_COINS_TO_ACCUMULATE - (i+1)]->getPublicCoin();
...
Test_MintCoin()
{
	...
	try {
		...
		gCoinSize /= TEPQI_COINS_TO_ACCUMULATE;
